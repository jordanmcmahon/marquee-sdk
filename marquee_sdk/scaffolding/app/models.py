...
class Category(HasCoverContent):
    ...
    def toJSONSafe(self, link_root=''):
        return {
            'title'             : self.title,
            'link'              : link_root + self.link,
            'id'                : self.id,
            'slug'              : self.slug,
            'cover': {
                '640'   : self.cover(640),
                '1280'  : self.cover(1280),
            }
        }
...
class Story(MContentModel, HasCoverContent):
    ...
    def __init__(self, *args, **kwargs):
        ...
        # Default category, for URL purposes.
    ...
    def published(self):
        ...
        it may change to be a query like `?_as_of=@published_date` once the
        history feature stabilizes, so this abstraction will keep the template
        API consistent.
    ...
    @property
    def link(self):
        return "/{0}/{1}/".format(self.category, self.slug)

    def embedCover(self, as_obj=False):
        if isinstance(self.cover_content, dict) and 'embed' in self.cover_content:
            embed_obj = self.cover_content['embed']
            if as_obj:
                return embed_obj
            return embed_obj.get('content', '')
        return ''
    ...
    def toJSONSafe(self, link_root=''):
        return {
            ...
            'tags'              : self.get('tags'),
            'published_date'    : dateToString(self.first_published_date),
            'updated_date'      : dateToString(self.published_date),
            'cover': {
                '640'   : self.cover(640),
                '1280'  : self.cover(1280),
            }
